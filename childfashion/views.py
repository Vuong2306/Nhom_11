...
from .forms import TheLoai1Form,TheLoai2Form,ThemLoaiForm,RegistrationForm,BangSPForm
...
def index(request):
   data = {"DMLoai": Loai.objects.all()}
   return render(request, 'pages/home.html', data)
def login(request):
    if request.method == 'POST':
        # Xác thực người dùng
        # Nếu xác thực thành công, lấy URL mục tiêu (nếu có)
...
def register(request):  
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            return HttpResponseRedirect('/childfashion/login')
...
def chitietsp(request, product_id):
    ...
    sizes = product.KichThuoc.split(', ') if product.KichThuoc else []
    colors = product.MauSac.split(', ') if product.MauSac else []

    return render(request, 'pages/ChiTietSP.html', {
        'product': product,
        'sizes': sizes,
        'colors': colors,
    })
...
def ThemSP(request):
    if request.method == "POST":
        form = BangSPForm(request.POST, request.FILES)
        if form.is_valid():
            bangsp = form.save(commit=False)
            bangsp.KichThuoc = ', '.join(request.POST.getlist('size'))  # Đổi 'KichThuoc' thành 'size'
            bangsp.MauSac = ', '.join(request.POST.getlist('color'))   # Đổi 'MauSac' thành 'color'
            bangsp.save()
            return HttpResponseRedirect('dssp1')  # Đảm bảo tên là 'dssp1'
    else:
        form = BangSPForm()
